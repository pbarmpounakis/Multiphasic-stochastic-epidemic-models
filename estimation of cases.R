...
DPcode_poisson <- nimbleCode(
  { 
    ...
    for(i in (31):N) {
      cases[i] ~ T(dnorm(cases[i-1],sd=sd),0,)
    }
    sd~dgamma(1,0.000001) 
  }
)
...
CDPmodel_MCMCConf_poisson$addMonitors(c('cases','mu_deaths'))
...
save(CDPmodel_samples_poisson,file='C:\\Users\\barbounakis\\OneDrive - aueb.gr\\PhD\\Coding\\Covid-19\\Branching process\\Nimble\\Final Renewal equations - NB likelihood easy sim\\Estimation of cases\\Final simple model alone, similar timeframe with the other models\\sim_simple_model.RData')
...
plot_filepath<-"C:\\Users\\pbarb\\OneDrive - aueb.gr\\PhD\\Coding\\Covid-19\\Branching process\\Nimble\\Final Renewal equations - NB likelihood easy sim\\Estimation of cases\\Final simple model alone, similar timeframe with the other models\\plots\\"
plot_index<-c('df45','df35','df25','df15')

ggsave(filename=paste0(plot_filepath,'SIM_cases_95CrI_nosmoothing.tiff'),device='tiff',dpi=300,width=10,height=9)
ggsave(filename=paste0(plot_filepath,'SIM_cases_95CrI_nosmoothing.jpeg'),device='jpeg',dpi=300,width=10,height=9)
...
ggsave(filename=paste0(plot_filepath,'SIM_deaths_95CrI.tiff'),device='tiff',dpi=300,width=10,height=9)
ggsave(filename=paste0(plot_filepath,'SIM_deaths_95CrI.jpeg'),device='jpeg',dpi=300,width=10,height=9)
...
for(i in 1:length(level_smoothing)){
  ...
  ggsave(filename=paste0(plot_filepath,'SIM_cases_95CrI_',plot_index[i],'.tiff'),device='tiff',dpi=300,width=10,height=9)
  ggsave(filename=paste0(plot_filepath,'SIM_cases_95CrI_',plot_index[i],'.jpeg'),device='jpeg',dpi=300,width=10,height=9)
