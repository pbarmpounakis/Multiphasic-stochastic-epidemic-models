...
run_MCMC_NB <- function(seed,data,constants,nburnin,niter,thin) {
  ...
  inits<-  list(R = runif(constants$Trunc,0,3),
    ...
    PYcode<- nimbleCode(
    {
      ...
      for(i in 1:(Trunc-1)) { # stick-breaking variables
        v[i] ~ dbeta(1-alpha, theta+i*alpha)
      }
  ...
  PYmodel <- nimbleModel(PYcode, data = data,inits = inits, constants = constants) 
  
  CPYmodel_MCMCConf <- configureMCMC(PYmodel, print = TRUE,enableWAIC = TRUE)
  
  
  CPYmodel_MCMCConf$addMonitors(c('Rt','z','w'))
  
  PYmodel_MCMC<- buildMCMC(CPYmodel_MCMCConf)
  
  CPYmodel<- compileNimble(PYmodel,showCompilerOutput = TRUE)
  
  CPYmodel_MCMC <- compileNimble(PYmodel_MCMC, project = PYmodel)
  
  CPYmodel_samples <- runMCMC(CPYmodel_MCMC, nburnin = nburnin, niter = niter,thin=thin, setSeed = seed,summary=T,WAIC=T)
  
  return(CPYmodel_samples)
}
...
run<-parLapply(cl = this_cluster, X = seq(10,80,10), 
               fun = run_MCMC_NB,
...
save(run,file='C:\\Users\\barbounakis\\OneDrive - aueb.gr\\PhD\\Coding\\Covid-19\\Branching process\\Nimble\\Final Renewal equations - NB likelihood easy sim\\PY\\Stick\\PY_NB_sim_daily_infections.RData')

run_MCMC_Poisson <- function(seed,data,constants,inits,nburnin,niter,thin) {
  library(nimble)
  ...
  CPYmodel_samples <- runMCMC(CPYmodel_MCMC, nburnin = nburnin, niter = niter,thin=thin, setSeed = 1,summary=T,WAIC=T)
...
run_poisson<-parLapply(cl = this_cluster, X = seq(10,80,10), 
               fun = run_MCMC_Poisson,
               data = data,constants=constants,inits=inits,nburnin = 10000, niter =50000,thin=10)
end_time<-Sys.time()
stopCluster(this_cluster)  

print(end_time-start_time)
save(run_poisson,file='C:\\Users\\barbounakis\\OneDrive - aueb.gr\\PhD\\Coding\\Covid-19\\Branching process\\Nimble\\Final Renewal equations - NB likelihood easy sim\\PY\\Stick\\PY_Poisson_sim_daily_infections.RData')
...
print(ggarrange(grph[[1]], grph[[2]], grph[[3]], grph[[4]],   
                ...
                ncol = 2, nrow = 2))
plot_filepath<-"C:\\Users\\barbounakis\\OneDrive - aueb.gr\\PhD\\Coding\\Covid-19\\Branching process\\Nimble\\Final Renewal equations - NB likelihood easy sim\\PY\\Stick\\plots Rt estimations\\simulation\\"
ggsave(filename=paste0(plot_filepath,'Daily_infections_PY_multiple_chains.tiff'),device='tiff',dpi=300,width=10,height=9)
ggsave(filename=paste0(plot_filepath,'Daily_infections_PY_multiple_chains.jpeg'),device='jpeg',dpi=300,width=10,height=9)
...
ggsave(filename=paste0(plot_filepath,'Daily_infections_PY_Rt_chains_combined.tiff'),device='tiff',dpi=300,width=10,height=9)
ggsave(filename=paste0(plot_filepath,'Daily_infections_PY_Rt_chains_combined.jpeg'),device='jpeg',dpi=300,width=10,height=9)
