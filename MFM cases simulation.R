...
run_MCMC_allcode <- function(seed,data,constants,nburnin,niter,thin) {
  ...
  inits<-  list(R = runif(constants$Kmax,0,3),
                ...
                e=rep(1,constants$Kmax)/constants$Kmax,
                lambda=0.02
  ) 
  get_weights<-nimbleFunction(  run = function(e=double(1)) {
    returnType(double(1))
    k<-length(e)
    cumsum_e<-rep(0,k)
    w<-rep(0,k)
    K_tilde<-1
    for(i in 1:k) {
      cumsum_e[i]=sum(e[1:i])
      if(cumsum_e[i]<1){
        K_tilde<-i+1
      }
    }
    w[1:(K_tilde-1)]<-e[1:(K_tilde-1)]
    w[K_tilde]<-1-sum(e[1:(K_tilde-1)])
    
    return(w)
  })
  assign('get_weights', get_weights, envir = .GlobalEnv)
  get_K<-nimbleFunction(  run = function(e=double(1)) {
...
CMFMmodel_MCMCConf$addMonitors(c('Rt','K','e','w'))
...
save(run3,file='C:\\Users\\barbounakis\\OneDrive - aueb.gr\\PhD\\Coding\\Covid-19\\Branching process\\Nimble\\Final Renewal equations - NB likelihood easy sim\\MFM\\MFM_NB_sim_representation3.RData')
...
for(j in 1:8){
  ...
  ggsave(filename=paste0(plot_filepath,'Daily_infections_as_data_MFM_multiple_chains.tiff'),device='tiff',dpi=300,width=10,height=9)
  ggsave(filename=paste0(plot_filepath,'Daily_infections_as_data_MFM_multiple_chains.jpeg'),device='jpeg',dpi=300,width=10,height=9)
...
dat_Rt<-data.frame(real_Rt=Rt_real,est_Rt=Rt_results_all_chains[(31+starting_day):(31+length(cases_daily)),3],est_Rt_low=Rt_results_all_chains[(31+starting_day):(31+length(cases_daily)),4],est_Rt_high=Rt_results_all_chains[(31+starting_day):(31+length(cases_daily)),5])
  ...
  ggsave(filename=paste0(plot_filepath,'Daily_infections_MFM_Rt_chains_combined_blacked.tiff'),device='tiff',dpi=300,width=10,height=9)
  ggsave(filename=paste0(plot_filepath,'Daily_infections_MFM_Rt_chains_combined_blacked.jpeg'),device='jpeg',dpi=300,width=10,height=9)
